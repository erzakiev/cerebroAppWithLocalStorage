...
## projection).
##----------------------------------------------------------------------------##
expression_projection_selected_cells_coords <- reactive({
  ...
  if (
    is.null(plotly::event_data("plotly_brushing", source = "expression_projection")) ||
    length(plotly::event_data("plotly_brushing", source = "expression_projection")) == 0
  ) {
    return(NULL)
    ## ... selection has been made and at least 1 cell is in it
  } else {
    ## get number of selected cells
    selected_cells_coords <- plotly::event_data("plotly_brushing", source = "expression_projection")
