...
prettifyTable <- function(
  ...
  page_length_menu = c(15, 30, 50, 100, 1000, 10000, 100000)
) {
...
getXYranges <- function(table) {
  if (ncol(table) < 3){
    ranges <- list(
      x = list(
        min = table[,1] %>% min(na.rm=TRUE) %>% "*"(ifelse(.<0, 1.2, 0.8)) %>% round(digits = 2),
        max = table[,1] %>% max(na.rm=TRUE) %>% "*"(ifelse(.<0, 0.8, 1.2)) %>% round(digits = 2)
      ),
      y = list(
        min = table[,2] %>% min(na.rm=TRUE) %>% "*"(ifelse(.<0, 1.2, 0.8)) %>% round(digits = 2),
        max = table[,2] %>% max(na.rm=TRUE) %>% "*"(ifelse(.<0, 0.8, 1.2)) %>% round(digits = 2)
      ),
      z = list(
        min = -0.5,
        max = 0.5
    ...
    ranges <- list(
      ...
      z = list(
        min = table[,3] %>% min(na.rm=TRUE) %>% "*"(ifelse(.<0, 1.2, 0.8)) %>% round(digits = 2),
        max = table[,3] %>% max(na.rm=TRUE) %>% "*"(ifelse(.<0, 0.8, 1.2)) %>% round(digits = 2)
      )
    )
  }
